...
class model_zambezi:
    ...
    def simulate(self):
        ...
        for t in range(self.H):
            ...
            mass_balance_ReservoirSim['itt'].write( str(q_Itt) + " " + str(h_itt[t]) + " " + str(s_itt[t]) + " " + str(s_itt[t+1]) + " " + str(r_itt[t+1]) + " " + str(r_itt_delay[t+1]) + "\n") 
            mass_balance_ReservoirSim['kgu'].write( str(q_KafueFlats+r_itt_delay[t+1]-r_irr4[t+1])+ " " + str(h_kgu[t]) + " " + str(s_kgu[t]) + " " + str(s_kgu[t+1]) + " " + str(r_kgu[t+1]) + '\n') # 
            mass_balance_ReservoirSim['kgl'].write( str(r_kgu[t+1]) + " " + str(h_kgl[t]) + " " + str(s_kgl[t]) + " " + str(s_kgl[t+1]) + " " + str(r_kgl[t+1]) + '\n') # 
            mass_balance_ReservoirSim['ka'].write( str(q_Bg+q_Cuando+q_KaLat-r_irr2[t+1])  + " " + str(h_ka[t]) + " " + str(s_ka[t]) + " " + str(s_ka[t+1]) + " " + str(r_ka[t+1]) + '\n') # 
            mass_balance_ReservoirSim['cb'].write(str(q_Cb+r_kgl[t+1]+r_ka[t+1]-(r_irr3[t+1]+r_irr5[t+1]+r_irr6[t+1])) + " " + str(h_cb[t]) + " " + str(s_cb[t]) + " " + str(s_cb[t+1]) + " " + str(r_cb[t+1]) + '\n') # 
            
            rDelta.write( str(r_cb[t+1]-r_irr7[t+1]-r_irr8[t+1]+q_Shire-r_irr9[t+1]) + '\n')
            irrigation.write(str(r_irr2[t+1]) + " " + str(r_irr3[t+1]) + " " + str(r_irr4[t+1]) + " " + str(r_irr5[t+1]) + " " + str(r_irr6[t+1]) + " " + str(r_irr7[t+1]) + " " + str(r_irr8[t+1]) + " " + str(r_irr9[t+1]) + '\n')        
